...
# df_X = df_X.shuffle(seed=35).select(range(min(len(df_X), 10000)))
...
#STAGE 2: SUPERVISED FINETUNE STEP
def preprocess_supervised(label_df):
...
peft_model = PeftModel.from_pretrained(adapted_model, f"{output_dir}/unsupervised-finetuned")
...
preprocessed_df = df_y.map(preprocess_supervised, desc="preprocessing", remove_columns=df_y.column_names)
df = dict()
